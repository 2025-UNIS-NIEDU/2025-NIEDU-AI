...
# === 메인 함수 ===
def generate_multi_quiz():
    """뉴스 요약 기반 N단계 + I단계 다지선다 퀴즈 자동 생성"""
    # === 세션 선택 ===
    ...
    def generate_quiz_i(n_quiz, summary):
        ...
        - 해설(explanation): 최소 40자, 50자 내외, 정답 근거 중심
    ...
    def strip_debug_info(quiz_list):
        clean_list = []
        for q in quiz_list:
            q_clean = {"question": q.get("question"), "answers": q.get("answers")}
            if "explanation" in q:
    ...
    file_path = SAVE_DIR / f"{topic}_multi_ni_{today}.json"
